...
server.get('/workers/chauffeur/generate/:amount', async ({ params }) => {
    ...
    for (let index = 0; index < parseInt(params.amount); index++) {
        ...
        for (let j = 0; j < Math.floor(Math.random() * 20); j++) {
            let latestHour = (Date.now() - Date.now() % (msPerHour));
            let start = latestHour + msPerHour * (Math.floor(Math.random() * 6) + 22) * j;
            let end = start + msPerHour * (Math.floor(Math.random() * 2) + 6);
...
server.get('/workers/picker/generate/:amount', async ({ params }) => {
    ...
    for (let index = 0; index < parseInt(params.amount); index++) {
        ...
        for (let j = 0; j < Math.floor(Math.random() * 20); j++) {
            let start = (Date.now() - Date.now() % msPerHour) + msPerHour * (Math.floor(Math.random() * 6) + 22);
            let end = start + msPerHour * (Math.floor(Math.random() * 2) + 6);
    ...
    for (let index = 0; index < parseInt(params.amount); index++) {
        ...
        for (let j = 0; j < Math.floor(Math.random() * productNames1.length); j++) {
            warehouse.stock.push({
                productId: Math.floor(Math.random() * (logisticsSystemConfig?.nextProductId || 1) - 1),
                shelf: j,
            ...
            if (wareHouseSources.length > 1) {
                for (let j = 1; j < wareHouseSources.length; j++) {
                    ...
                    for (let k = 0; k < Math.floor(Math.random() * 5); k++) {
                        subOrder.status.push({ timeStamp: new Date(Date.now() - msPerHour * 4 + msPerHour * k + Math.floor(Math.random() * 1800 * 1000)) });
                        if (k == 1) {
                            ...
                            if (possiblePickers.length > 0) {
                                subOrder.status[k].WorkerId = possiblePickers[Math.floor(Math.random() * (possiblePickers.length - 1))].workerId;
                            }
                            else {
                        ...
                        if (k == 3) {
                            let possibleChauffeurs = await Chauffeur.find({
                                schedule: { $elemMatch: { start: { $lte: subOrder.status[k].timeStamp },
                                end: { $gte: subOrder.status[k].timeStamp}
                            }}});
                            if (possibleChauffeurs.length > 0) {
                                subOrder.status[k].WorkerId = possibleChauffeurs[Math.floor(Math.random() * (possibleChauffeurs.length - 1))].workerId;
                            }
                            else {
                ...
                for (let k = 0; k < Math.floor(Math.random() * 5); k++) {
                    order.status.push({ timeStamp: new Date(Date.now() - msPerHour * 4 + msPerHour * k + Math.floor(Math.random() * 1800 * 1000)) });
                    if (k == 1) {
                        let possiblePickers = await Picker.find({ schedule: { $elemMatch: { start: { $lte: order.status[k].timeStamp }, end: { $gte: order.status[k].timeStamp }, wareHouse: order.warehouseId } } });
                        if (possiblePickers.length > 0) {
                    ...
                    if (k == 3) {
                        let possibleChauffeurs = await Chauffeur.find({ schedule: { $elemMatch: { start: { $lte: order.status[k].timeStamp }, end: { $gte: order.status[k].timeStamp } } } });
                        if (possibleChauffeurs.length > 0) {
...
mongoose.connect(`mongodb+srv://${Bun.env.MONGOOSE_USERNAME}:${Bun.env.MONGOOSE_PASSWORD}@logisticssystem.1yypplx.mongodb.net/Kitty?retryWrites=true&w=majority`).then(() => console.log("Connected"));